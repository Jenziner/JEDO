...
for ORGANIZATION in $ORGANIZATIONS; do
    # Params
    TOOLS_NAME=$(yq eval ".Organizations[] | select(.Name == \"$ORGANIZATION\") | .Tools.Name" "$CONFIG_FILE")
    TOOLS_PASS=$(yq eval ".Organizations[] | select(.Name == \"$ORGANIZATION\") | .Tools.Pass" "$CONFIG_FILE")
    TOOLS_IP=$(yq eval ".Organizations[] | select(.Name == \"$ORGANIZATION\") | .Tools.IP" "$CONFIG_FILE")
    TOOLS_PORT=$(yq eval ".Organizations[] | select(.Name == \"$ORGANIZATION\") | .Tools.Port" "$CONFIG_FILE")
