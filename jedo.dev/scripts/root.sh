...
docker run -d \
    ...
    sh -c "fabric-ca-server start -b $ROOT_TLSCA_ADMIN_NAME:$ROOT_TLSCA_ADMIN_PASS" 
...
# Enroll Root-TLS-Admin
echo ""
echo_info "Root-TLS-Admin enrolling..."
docker exec -it $ROOT_TLSCA_NAME fabric-ca-client enroll -u https://$ROOT_TLSCA_ADMIN_NAME:$ROOT_TLSCA_ADMIN_PASS@$ROOT_TLSCA_NAME:$ROOT_TLSCA_PORT
...
docker exec -it $ROOT_TLSCA_NAME fabric-ca-client affiliation add $AFFILIATION_ROOT -u https://$ROOT_TLSCA_ADMIN_NAME:$ROOT_TLSCA_ADMIN_PASS@$ROOT_TLSCA_NAME:$ROOT_TLSCA_PORT
...
docker exec -it $ROOT_TLSCA_NAME fabric-ca-client affiliation remove org1 --force -u https://$ROOT_TLSCA_ADMIN_NAME:$ROOT_TLSCA_ADMIN_PASS@$ROOT_TLSCA_NAME:$ROOT_TLSCA_PORT
docker exec -it $ROOT_TLSCA_NAME fabric-ca-client affiliation remove org2 --force -u https://$ROOT_TLSCA_ADMIN_NAME:$ROOT_TLSCA_ADMIN_PASS@$ROOT_TLSCA_NAME:$ROOT_TLSCA_PORT
...
ROOT_ORGCA_ADMIN_NAME=$(yq eval ".Root.ORG-CA.Admin" "$CONFIG_FILE")
ROOT_ORGCA_ADMIN_PASS=$(yq eval ".Root.ORG-CA.Pass" "$CONFIG_FILE")
...
docker run -d \
    ...
    -e FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_SRV_DIR/tls-cert.pem \
    hyperledger/fabric-ca:latest \
    sh -c "fabric-ca-server start -b $ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS" 
...
# Enroll Root-ORG-Admin
echo ""
echo_info "Root-ORG-Admin enrolling..."
docker exec -it $ROOT_ORGCA_NAME fabric-ca-client enroll -u https://$ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS@$ROOT_ORGCA_NAME:$ROOT_ORGCA_PORT
...
AFFILIATION_ROOT=$TLDN
docker exec -it $ROOT_ORGCA_NAME fabric-ca-client affiliation add $AFFILIATION_ROOT -u https://$ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS@$ROOT_ORGCA_NAME:$ROOT_ORGCA_PORT
AFFILIATION_ROOT=$TLDN.$DN
docker exec -it $ROOT_ORGCA_NAME fabric-ca-client affiliation add $AFFILIATION_ROOT -u https://$ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS@$ROOT_ORGCA_NAME:$ROOT_ORGCA_PORT
...
echo ""
echo_info "Default affiliation removing..."
docker exec -it $ROOT_ORGCA_NAME fabric-ca-client affiliation remove org1 --force -u https://$ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS@$ROOT_ORGCA_NAME:$ROOT_ORGCA_PORT
docker exec -it $ROOT_ORGCA_NAME fabric-ca-client affiliation remove org2 --force -u https://$ROOT_ORGCA_ADMIN_NAME:$ROOT_ORGCA_ADMIN_PASS@$ROOT_ORGCA_NAME:$ROOT_ORGCA_PORT
