...
###############################################################
# Integration Test: Gateway → CA Service → Ledger Service
###############################################################
...
OUTPUT_DIR="$DOWNLOADS_DIR/jedo-integration-test-$TEST_RUN_ID"
mkdir -p "$OUTPUT_DIR"
...
# Gateway API endpoints (proxied)
GATEWAY_CA_REGISTER="${GATEWAY_URL}/api/v1/ca/certificates/register"
...
# Temp dir
TEMP_CERTS_DIR="/tmp/jedo-integration-test"
...
test_ca_health() {
    echo_section "TEST 2: CA-Service Health Check"
...
test_ledger_health() {
    echo_section "TEST 3: Ledger-Service Health Check"
...
test_gateway_ready() {
    echo_section "TEST 4: Gateway Readiness Check (Backend Services)"
...
register_gens_via_gateway() {
    echo_section "TEST 5: Register Gens via Gateway (as Ager)"
...
EOF
    ...
    echo_info "Sending request via Gateway..."

    response=$(curl -k -s -w "\n%{http_code}" \
        -X POST "${GATEWAY_CA_REGISTER}" \
...
enroll_gens_via_gateway() {
    echo_section "TEST 6: Enroll Gens via Gateway (X.509)"
...
register_human_via_gateway() {
    echo_section "TEST 7: Register Human via Gateway (as Gens)"
...
enroll_human_idemix_via_gateway() {
    echo_section "TEST 8: Enroll Human via Gateway (Idemix)"
...
EOF
    ...
    if [ "$http_code" -eq 200 ]; then
        echo_info "✅ Human enrolled via Gateway (Idemix)"
...
main() {
    echo ""
    echo "========================================"
    echo "  JEDO GATEWAY INTEGRATION TEST"
    echo "========================================"
    echo ""
    ...
    echo "  ✅ Gateway (Health + Readiness)"
    echo "  ✅ CA-Service (via Gateway)"
    echo "  ✅ Ledger-Service (Health)"
    echo ""
