...
#!/bin/bash
#
# This script enrolls Org-MSP Material.
#
###############################################################
set -euo pipefail
...
log_section "JEDO-Ecosystem - Org-MSP enrolling..."
...
AGER_ORG_NAME=$(yq eval '.Ager.msp.Name' "${CERTS_CONFIGFILE}")
AGER_ORG_NAME=$AGER_ORG_NAME.$AGER_NAME.$REGNUM_NAME.$ORBIS_NAME.$ORBIS_TLD
AGER_ORG_CSR="C=XX,ST=$ORBIS_ENV,L=$REGNUM_NAME,O=$AGER_NAME"

LOCAL_CAROOTS_DIR="${SCRIPTDIR}/../../infrastructure/tls-ca-roots"
HOST_CAROOTS_DIR="/etc/hyperledger/tls-ca-roots"

LOCAL_CLIENT_DIR="${SCRIPTDIR}/../../infrastructure/clients/${AGER_ORG_NAME}"
...
log_debug "ORG MSP Info:" "$AGER_ORG_NAME / $AGER_ORG_CSR"
...
log_info "$AGER_ORG_NAME enrolling..."
...
docker run --rm \
  ...
  fabric-ca-client enroll \
      -u https://${AGER_ORG_NAME}:${AGER_ORG_PASS_RAW}@$REGNUM_MSP_NAME:$REGNUM_MSP_PORT \
      --tls.certfiles ${HOST_CAROOTS_DIR}/${REGNUM_TLS_NAME}.pem \
      --mspdir ${HOST_CLIENT_DIR}/msp \
      --csr.hosts ${AGER_ORG_NAME} \
      --csr.cn $AGER_ORG_NAME --csr.names "$AGER_ORG_CSR"
log_debug "ORG-MSP enrolled"
...
log_ok "Org-MSP $AGER_ORG_NAME enrolled."
