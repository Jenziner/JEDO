...
###############################################################
#
# JEDO-Ecosystem - TLS-CA Bootstrap Enrollment
#
############################################################### 
set -euo pipefail
...
# Run
###############################################################
log_section  "JEDO-Ecosystem - TLS-CA Bootstrap Identity Enrollment"
...
docker run --rm \
    ...
    fabric-ca-client enroll \
        -u https://bootstrap.${TLS_CA_NAME}:${TLS_CA_BOOTSTRAP_PASS}@${TLS_CA_NAME}:${TLS_CA_PORT} \
...
docker run --rm \
    --network ${DOCKER_NETWORK} \
    -v "${LOCAL_CAROOTS_DIR}:${HOST_CAROOTS_DIR}" \
    -v "${LOCAL_TLS_SRV_DIR}:${HOST_SRV_DIR}" \
    -v "${LOCAL_TLS_CLIENT_DIR}:${HOST_CLIENT_DIR}" \
    -e FABRIC_CA_CLIENT_HOME="${HOST_CLIENT_DIR}" \
    -e FABRIC_CA_CLIENT_LOGLEVEL=${FABRIC_CA_CLIENT_LOGLEVEL} \
    ${FABRIC_CA_IMAGE} \
...
# =============================================================
CA_CERT_FILE=$(ls ${LOCAL_TLS_SRV_DIR}/msp/cacerts/*.pem | head -n 1)
CA_CERT_BASENAME=$(basename "$CA_CERT_FILE")

cat > "${LOCAL_TLS_SRV_DIR}/msp/config.yaml" <<EOF
NodeOUs:
  Enable: true
  ClientOUIdentifier:
...
log_info "TLS-CA Bootstrap Identity Enrollment completed."