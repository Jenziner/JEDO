...
func (s *SmartContract) CreateWallet(ctx contractapi.TransactionContextInterface, walletID string, ownerID string, initialBalance float64, metadataJSON string) error {
	...
	if gensName != "" && !strings.Contains(ownerID, "."+gensName+".") && !strings.HasPrefix(ownerID, gensName+".") {
	...
	if initialBalance > 0 {
		...
		err = ctx.GetStub().PutState(txKey, txJSON)
		if err != nil {
	...
	ctx.GetStub().SetEvent("WalletCreated", eventJSON)
	return nil
}
